...
def _coerce_llm_content_to_text(content: Any) -> str:
    """Convert provider-specific message content blocks into plain text."""
    if content is None:
        return ""
    ...
    if isinstance(content, list):
        ...
        for block in content:
            ...
            if isinstance(block, dict):
                ...
                if btype in (None, "text", "output_text", "redacted_text"):
                    part = block.get("text") or block.get("content") or ""
                    text_parts.append(_coerce_llm_content_to_text(part))
                continue
            for attr in ("text", "content"):
                part = getattr(block, attr, None)
                if part:
                    text_parts.append(_coerce_llm_content_to_text(part))
                    break
...
def _canonicalize_agent_tags(message: str) -> str:
    """Normalize common XML tag variants emitted by smaller/chatty models."""
    ...
    return re.sub(
        r"<\s*(/?)\s*(execute|solution|think)\s*>",
        replace_tag,
        message,
        flags=re.IGNORECASE,
    )
...
def _looks_like_unfinished_plan(message: str) -> bool:
    """Detect plan/checklist-only responses that should not end an agent run."""

    text = re.sub(r"<[^>]+>", "", str(message or "")).strip()
    if not text:
        return False

    unchecked_boxes = len(re.findall(r"\[\s*\]", text))
    plan_markers = [
        "分析框架",
        "需要检索",
        "检索的数据库",
    ...
    has_plan_marker = unchecked_boxes >= 2 or any(marker.lower() in text.lower() for marker in plan_markers)
    ...
    has_decisive_marker = any(marker.lower() in text.lower() for marker in decisive_markers)
    return has_plan_marker and not has_decisive_marker


PLAN_ONLY_CONTINUATION = """你刚才只输出了分析框架、待办清单或检索计划，这不能作为最终答案。
请继续完成任务，而不是重新给框架。

如果无法实际联网检索，请明确说明证据限制，并基于已有知识给出 provisional 判断。
最终答案必须包含：
1. 最终判断：Go / No-Go / Conditional Go（三选一）
2. 关键依据
3. 关键风险和证据缺口
4. 50万-100万元人民币预算下必须做的实验与不值得做的实验
5. 下一步建议

必须用 <solution>...</solution> 包裹最终答案。不要再次只输出计划。"""
...
class A1:
    def __init__(
        ...
        def mask_secret(value):
            value = str(value)
            return "*" * 8 + value[-4:] if len(value) > 8 else "***"
        ...
        for key, value in config_dict.items():
            if value is not None:
                ...
                if key == "commercial_mode":
                    ...
                    print(f"  {key.replace('_', ' ').title()}: {mask_secret(value)}")
                else:
...
"""
            ...
            prompt_modifier += """
FAST MODE:
For general explanation, brainstorming, and experimental design questions, answer directly from general biomedical knowledge. Do not run protocol searches, web searches, or data-lake analysis unless the user explicitly asks for them.
"""
  ...
  - Bash scripts and commands: Use the #!BASH marker in your execute block for both simple commands and complex shell scripts with variables, loops, conditionals, etc.
    ...
    def configure(self, self_critic=False, test_time_scale_round=0):
        ...
        if not self.use_tool_retriever:
            print("Fast prompt mode: skipping full tool/data/library catalogs for faster API calls")
            tool_desc = {}
            data_lake_with_desc = []
            library_content_list = []
            know_how_docs = []
        ...
        def generate(state: AgentState) -> AgentState:
            ...
            msg = _canonicalize_agent_tags(_coerce_llm_content_to_text(response.content)).strip()

            # Enhanced parsing for better OpenAI compatibility
            # Check for incomplete tags and fix them
            if re.search(r"<execute>", msg, re.IGNORECASE) and not re.search(r"</execute>", msg, re.IGNORECASE):
                msg += "</execute>"
            if re.search(r"<solution>", msg, re.IGNORECASE) and not re.search(r"</solution>", msg, re.IGNORECASE):
                msg += "</solution>"
            if re.search(r"<think>", msg, re.IGNORECASE) and not re.search(r"</think>", msg, re.IGNORECASE):
                msg += "</think>"
            ...
            # Alternative patterns for models that might use markdown code blocks
            if not execute_match:
                code_block_match = re.search(
                    r"```(?P<lang>python|py|bash|sh|r)?\s*(?P<code>.*?)```",
                    msg,
                    re.DOTALL | re.IGNORECASE,
                )
                if code_block_match and not answer_match:
                    code = code_block_match.group("code").strip()
                    lang = (code_block_match.group("lang") or "").lower()
                    if lang in {"bash", "sh"} and not code.startswith("#!BASH"):
                        code = "#!BASH\n" + code
                    elif lang == "r" and not code.startswith("#!R"):
                        code = "#!R\n" + code
                    thinking = msg[: code_block_match.start()].strip()
                    msg = f"{thinking}\n<execute>\n{code}\n</execute>".strip()
                    execute_match = re.search(r"<execute>(.*?)</execute>", msg, re.DOTALL | re.IGNORECASE)

            # If a smaller model returns a plain final answer, do not spin in parser repair loops.
            if not execute_match and not answer_match and msg.strip():
                visible_response = msg
                if think_match:
                    visible_response = (msg[: think_match.start()] + msg[think_match.end() :]).strip()
                if visible_response:
                    if _looks_like_unfinished_plan(visible_response):
                        state["messages"].append(AIMessage(content=msg.strip()))
                        state["messages"].append(HumanMessage(content=PLAN_ONLY_CONTINUATION))
                        state["next_step"] = "generate"
                        return state
                    msg = f"{think_match.group(0)}\n<solution>{visible_response}</solution>" if think_match else f"<solution>{visible_response}</solution>"
                answer_match = re.search(r"<solution>(.*?)</solution>", msg, re.DOTALL | re.IGNORECASE)
            ...
            if answer_match:
                ...
                error_count = sum(
                    ...
                    and "Each response must include thinking process" in str(m.content)
                )
        ...
        def execute(state: AgentState) -> AgentState:
            ...
            if execute_match:
                ...
                if not result.strip():
                    result = (
                        "No output captured from executed code. Use print(...) to show values, "
                        "or provide a <solution> if the task is already complete."
                    )
                ...
                # Feed execution output back as environment/user feedback, not as another assistant turn.
    ...
    def go(self, prompt):
        ...
        recursion_limit = int(os.getenv("BIOMNI_RECURSION_LIMIT", "80"))
        config = {"recursion_limit": recursion_limit, "configurable": {"thread_id": 42}}
        self.log = []
        ...
        def generate_response(prompt_input, inner_history=None, main_history=None):
            ...
            for s in self.app.stream(inputs, stream_mode="values", config=config):
                ...
                if isinstance(message.content, str):
                    ...
                    solution_match = re.search(r"<solution>(.*?)</solution>", message.content, re.DOTALL | re.IGNORECASE)
                    ...
                    execute_match = re.search(r"<execute>(.*?)</execute>", message.content, re.DOTALL | re.IGNORECASE)
                    if execute_match:
