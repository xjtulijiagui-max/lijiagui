...
这是一个给支行长和业务骨干使用的课堂营销作战台。气质稳、清楚、可信，像一组放在会议桌上的蓝色业务作战卡。页面按“智客发现经营与客户机会 → 智策形成方案 → 智战推进客户 → 智管封装团队Skill”展开，既保留可下载的数据练习，也提供即拿即用的销售提示词。
...
- **Variants**：客户/行业/经营洞察、销售录音复盘、解决方案/战略客户、拜访/赢单。
- **Layout**：桌面两列，移动单列；长提示词在卡片内独立滚动。

### Skill Conversion Exercise
- **Structure**：团队用户故事、提示词来源、Skill设计提示词、测试用例与30天试运行标准。
- **Purpose**：模块6只承接团队能力沉淀，不再放经营数据分析；经营考核和全国评比统一归入模块3。
...
完整样式内联在单文件HTML中，减少移动网络下的阻塞请求并避免异步样式造成布局偏移。离屏模块使用`content-visibility:auto`降低初始布局成本；正常滚动会触发渲染，打印样式与视觉QA捕获会强制完整呈现全部模块。
