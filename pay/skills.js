const skills = [
  ...
  ['S03', '交易经营数据分析与异常洞察', '完成数据质量检查、指标计算、趋势分析、异常识别和行动建议。', '经营分析、财务、产品、运营、管理层', '入门', '40 分钟', '你是支付公司经营分析师。分析附件数据，先检查数据质量和口径，再计算交易额、笔数、客单价、活跃商户、退款率和费率收入，按【区域/渠道/商户类型】比较。输出管理摘要、关键指标、趋势与异常、原因假设、三项行动建议和图表建议。'],
  ['S04', '差异账智能归因与处置建议', '结合规则和历史案例，对差异分类、归集证据并分流责任。', '清结算、财务、运营、技术支持', '进阶', '40 分钟', '你是清结算差异分析助手。依据【差异账明细】【规则】【历史案例】，逐笔判断差异类型，列出证据、置信度、缺失信息和处置路径。不得自动调账或关闭差异。最后统计高频根因并给出规则、流程、系统改进建议。'],
  ['S05', '协议规则解析与分润返佣校验', '将自然语言协议转成结构化规则并复算交易明细。', '渠道、财务、法务、产品运营', '进阶', '45 分钟', '你是协议规则数字化助手。从【协议文本】提取主体、范围、生效时间、费率、阶梯、封顶、返佣条件、结算周期、例外和优先级，形成规则表；再校验【计算明细】，逐条说明复算过程、差异和待确认项。每条规则引用原文，不得补造条款。'],
  ['S06', '客户画像、分群与营销机会识别', '基于合规可用特征进行可解释分群，生成下一最佳行动。', '销售、市场、客户运营、产品', '进阶', '40 分钟', '你是支付行业客户运营分析师。基于脱敏数据，围绕【增长/留存/交叉销售】建立画像维度并形成可解释分群。输出分群规则、各群特征、需求、机会与风险、下一最佳行动、客户经理任务和效果指标。不得使用未授权敏感属性。'],
  ['S07', '售后工单分析与智能应答', '对工单分类、定级、生成回复草稿和升级建议，并发现知识缺口。', '客服、售后、运营、产品支持', '入门', '35 分钟', '你是支付业务售后工单助手。依据【工单】【知识库】【SLA】，识别诉求、类型、紧急程度和情绪，给出处理步骤、回复草稿和升级建议。不得承诺未确认事项。最后汇总高频问题、根因假设和知识库补充建议。'],
...
function draw() {
  ...
  grid.innerHTML = visible.map((skill) => {
    const [id, title, description, role, level, duration, prompt] = skill;
    const file = encodeURIComponent(skillFilename(skill));
    return `<article class="skill-card" id="${id}"><div class="skill-top"><span class="skill-id">${id}</span><div class="badges"><span class="badge ${badgeClass(level)}">${level}</span><span class="badge">${duration}</span></div></div><h3>${title}</h3><p class="description">${description}</p><p class="role">适用对象：${role}</p><blockquote class="prompt">${safeText(prompt)}</blockquote><div class="card-actions"><button class="button" type="button" data-copy="${id}">复制提示词</button><a class="button download" href="downloads/${file}" download>下载独立 Skill ↓</a></div></article>`;
  }).join('');
}
