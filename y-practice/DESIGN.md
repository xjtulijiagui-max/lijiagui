...
这是一张面向企业课堂的“能力进阶工作台”。视觉延续上一场 WorkBuddy 公开课的蓝白体系，结构为模块 2、3、5、6 四关能力训练。页面不做营销落地页，也不使用装饰性大图；记忆签名是左侧连续进度线，以及每关都能产出一份可带走的文字成果。
...
- 平板隐藏顶部快捷导航，保留下方四关索引，避免导航文字挤压换行。
- 手机端标题按语义断行，模块内容保持单列可读。
...
- 四项未全部完成时禁用；完成后下载包含作业内容、四项完成状态和参考链接的 Markdown 学习记录。
